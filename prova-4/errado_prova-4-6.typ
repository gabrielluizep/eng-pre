...
$
E[X] = integral_(x in X) x P(X = x)
$

Para a distribuição $"Unif"({-1, 1})$ a probabilidade de que seja sorteado um
valor pertencente a cada ponto do intervalo $[-1, 1]$ é igual, graficamente
  ...
  #canvas(
    length: 1cm,
    {
      plot.plot(
        ...
        y-tick-step: 0.5,
        {
          plot.add(hypograph: true, style: plot.palette.blue, domain: (-1, 1), x => 1 / 2)
        },
      )
    },
  )
...
$
E[X_1] = E[X_2] &= E[X_3] = integral_(x in X) x P(X = x) \
&= integral_(-1)^(1) x dot.op 1/2 dif x \
&= 1/2 integral_(-1)^(1) x dif x \
&= 1/2 dot.op evaluated(x^2/2)_(-1)^(1) \
&= 1/2 dot.op [1^2/2 - ((-1)^2/2)] \
&= 1/2 dot.op (1/2 - 1/2) \
&= 1/2 dot.op 0 \
&= 0
$
